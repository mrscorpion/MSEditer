...
@interface MSFontSizePickerView () <UIScrollViewDelegate>

@end

@implementation MSFontSizePickerView
{
...
- (void)setDataSource:(id<MSFontSizePickerViewDataSource>)dataSource {
...
- (void)reloadData {
    ...
    for (NSInteger index = 0; index < self.numberOfItems; index++) {
        ...
        itemView.text = [self.dataSource MS_pickerView:self titleForItemAtIndex:index];
...
- (NSInteger)numberOfItems {
    return [self.dataSource MS_numberOfItemsInPickerView:self];
}
...
- (void)handleTap:(UITapGestureRecognizer *)tap {
    ...
    for (NSInteger index = 0; index < _itemViews.count; index++) {
        ...
        if (CGRectContainsPoint(label.frame, point)) {
            if (index != _selectedIndex && [self.delegate respondsToSelector:@selector(MS_pickerView:didSelectIndex:)]) {
                [self selectIndex:index animated:YES];
                [self.delegate MS_pickerView:self didSelectIndex:index];
            }
            else {
                [self selectIndex:index animated:YES];
            }
