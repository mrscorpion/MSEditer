...
#import "MSColorPickerView.h"

@interface MSColorPickerView ()

@end

@implementation MSColorPickerView
{
...
- (void)setDataSource:(id<MSColorPickerViewDataSource>)dataSource {
...
- (void)reloadData {
    ...
    for (NSInteger index = 0; index < self.numberOfColors; index++) {
        ...
        itemView.backgroundColor = [self.dataSource MS_colorPickerView:self colorForItemAtIndex:index];
...
- (NSInteger)numberOfColors {
    return [self.dataSource MS_numberOfColorsInColorPickerView:self];
}
...
- (void)handleTap:(UITapGestureRecognizer *)tap {
    ...
    for (NSInteger index = 0; index < _itemViews.count; index++) {
        ...
        if (CGRectContainsPoint(itemView.frame, point)) {
            ...
            if ([self.delegate respondsToSelector:@selector(MS_colorPickerView:didSelectIndex:)]) {
                [self.delegate MS_colorPickerView:self didSelectIndex:index];
            }
            if ([self.delegate respondsToSelector:@selector(MS_colorPickerView:didSelectColor:)]) {
                [self.delegate MS_colorPickerView:self didSelectColor:itemView.backgroundColor];
            }
            break;
        }
    }
